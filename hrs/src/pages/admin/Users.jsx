...
// ─── Benutzer-Detailpanel ────────────────────────────────────────────────────
function UserDetailPanel({ user, ranks, onClose, onRefresh }) {
  const [tab, setTab]       = useState('info') // 'info' | 'professions' | 'absence' | 'password'
  ...
  async function saveInfo() {
    setBusy(true); setErr('')
    try {
      await updateDoc(doc(db, 'users', user.id), { cls, race, level: Number(level), rank, characterType: charType })
      flash()
  ...
  async function saveProfs() {
    setBusy(true); setErr('')
    try {
      await updateDoc(doc(db, 'users', user.id), { professions: profs })
      flash()
    } catch (e) { setErr(e.message) }
    setBusy(false)
  }
  ...
  async function savePw() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'users', user.id), { passwordHash, password: null })
  ...
  const clsColor = CLASS_COLORS[cls] || '#c8a84b'
  const clsIcon  = CLASS_ICONS[cls]  || '⚔️'
  const absence  = user.absence

  const TABS = [
    { id: 'info',       label: 'Charakter' },
  ...
  function addProf() {
    ...
    if (!available) return
    setProfs([...profs, { name: available, level: 375, specialization: 'Keine' }])
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#120e06', border: '1px solid #4a3820', borderRadius: 4,
        ...
        <div style={{ padding: '1.4rem 1.4rem 0', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '1rem' }}>
            <div style={{ width: 44, height: 44, borderRadius: 3, background: '#0d0a04', border: `1px solid ${clsColor}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>
              {clsIcon}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'Cinzel,serif', fontSize: 16, color: clsColor, fontWeight: 600 }}>{user.name}</div>
              <div style={{ fontSize: 11, color: '#5a4828', marginTop: 2 }}>
                {rank} · {cls}{race ? ` · ${race}` : ''}{user.level ? ` · Level ${user.level}` : ''}
              </div>
              {absence && (
                <div style={{ fontSize: 10, color: '#e08080', marginTop: 3 }}>🏖️ Abwesend {absence.from}{absence.until ? ` – ${absence.until}` : ''}</div>
              )}
            </div>
            <button onClick={onClose} style={{ background: 'transparent', border: 'none', color: '#4a3820', fontSize: 18, cursor: 'pointer', padding: '4px 8px' }}>✕</button>
          </div>

          {/* Tabs */}
          <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid #2e2210', marginBottom: 0 }}>
            {TABS.map(t => (
        ...
        <div style={{ padding: '1.2rem 1.4rem', overflowY: 'auto', flex: 1 }}>
          ...
          {tab === 'info' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Klasse</div>
                ...
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Rasse</div>
                ...
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Level</div>
                ...
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Rang</div>
              ...
              <div>
                <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Charakter-Typ</div>
                <div style={{ display: 'flex', gap: 8 }}>
                  {[{ id: 'main', label: '⭐ Main' }, { id: 'twink', label: '🔄 Twink' }].map(opt => (
                    <button key={opt.id} onClick={() => setCharType(opt.id)} style={{
                      flex: 1, background: charType === opt.id ? 'rgba(200,168,75,.1)' : 'transparent',
                      border: charType === opt.id ? '1px solid #c8a84b' : '1px solid #2e2210',
                      borderRadius: 3, padding: '8px', cursor: 'pointer',
                      fontFamily: 'Cinzel,serif', fontSize: 11, letterSpacing: 1,
                      color: charType === opt.id ? '#f0d080' : '#5a4828', transition: 'all .15s',
                    }}>{opt.label}</button>
                  ))}
                </div>
              </div>
          ...
          {tab === 'professions' && (
            <div>
              {profs.length === 0
                ? <div style={{ textAlign: 'center', padding: '1.5rem', color: '#3a2c18', fontStyle: 'italic', fontSize: 12 }}>Keine Berufe eingetragen.</div>
                : profs.map((prof, idx) => {
                    const specs    = PROFESSIONS[prof.name] || ['Keine']
                    const hasSpecs = specs.length > 1
                    return (
                      <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 70px 1fr auto', gap: 8, alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid #1a1208' }}>
                        ...
                        {hasSpecs
                          ? <select value={prof.specialization || 'Keine'} onChange={e => { const n = [...profs]; n[idx] = { ...prof, specialization: e.target.value }; setProfs(n) }} style={{ fontSize: 11 }}>
                              {specs.map(s => <option key={s} value={s}>{s}</option>)}
                            </select>
                          : <div style={{ fontSize: 10, color: '#3a2c18', fontStyle: 'italic' }}>Keine Spezialisierung</div>
                        }
                        <button onClick={() => setProfs(profs.filter((_, i) => i !== idx))} style={{ background: 'transparent', border: 'none', color: '#4a2820', cursor: 'pointer', fontSize: 13 }}
                          onMouseEnter={e => e.target.style.color = '#c04040'} onMouseLeave={e => e.target.style.color = '#4a2820'}>✕</button>
                      </div>
                    )
                  })
              }
              {(canAddPrimary || canAddSecondary) && (
                <button className="btn-ghost" style={{ fontSize: 10, marginTop: 10 }} onClick={addProf}>+ Beruf hinzufügen</button>
              )}
              <div style={{ fontSize: 10, color: '#2e2210', fontStyle: 'italic', marginTop: 8 }}>{primaryCount}/2 Hauptberufe</div>
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {user.absence && (
                <div style={{ background: 'rgba(192,57,43,.08)', border: '1px solid #3a1a1a', borderRadius: 3, padding: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ fontSize: 12, color: '#e08080' }}>
                    🏖️ {user.absence.from} – {user.absence.until || 'offen'}
                    {user.absence.reason && <span style={{ color: '#6a3030', marginLeft: 8, fontStyle: 'italic' }}>{user.absence.reason}</span>}
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Von</div>
                  <input type="date" value={absFrom} onChange={e => setAbsFrom(e.target.value)} style={{ fontSize: 12, width: '100%', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Bis (optional)</div>
              ...
              <div>
                ...
                <input value={absReason} onChange={e => setAbsReason(e.target.value)} placeholder="z.B. Urlaub · Prüfungswoche" maxLength={80} style={{ fontSize: 12, width: '100%', boxSizing: 'border-box' }} />
              </div>
            </div>
          )}
          ...
          {tab === 'password' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p style={{ fontSize: 12, color: '#5a4828', fontStyle: 'italic', margin: 0 }}>Als Admin kannst du das Passwort direkt setzen ohne das alte zu kennen.</p>
              <div>
                <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Neues Passwort</div>
                <input type="password" value={newPw} onChange={e => setNewPw(e.target.value)} placeholder="Mind. 6 Zeichen" autoComplete="new-password" style={{ fontSize: 12, width: '100%', boxSizing: 'border-box' }} />
              </div>
              <div>
                <div style={{ fontFamily: 'Cinzel,serif', fontSize: 8, letterSpacing: 2, color: '#3a2c18', textTransform: 'uppercase', marginBottom: 5 }}>Wiederholen</div>
                <input type="password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} placeholder="••••••••" autoComplete="new-password" style={{ fontSize: 12, width: '100%', boxSizing: 'border-box' }} />
              </div>
            </div>
          )}
        ...
        {/* Footer */}
        <div style={{ padding: '1rem 1.4rem', borderTop: '1px solid #2e2210', flexShrink: 0, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button className="btn-primary" style={{ fontSize: 11 }} disabled={busy}
            onClick={() => {
              if (tab === 'info')        saveInfo()
              if (tab === 'professions') saveProfs()
              if (tab === 'absence')     saveAbsence()
              if (tab === 'password')    savePw()
            }}>
...
export default function Users() {
  const { users, loading, addUser, deleteUser, toggleActive, updateNote } = useUsers()
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr>
                {['Status', 'Charakter', 'Klasse / Rasse', 'Rang', 'Seit', 'Aktionen'].map(h => (
            ...
            <tbody>
              ...
              {filtered.map(u => (
                <tr key={u.id} style={{ borderBottom: '1px solid rgba(46,34,16,.5)', cursor: 'pointer', transition: 'background .1s' }}
                  ...
                  <td style={{ padding: '.65rem .8rem', fontWeight: 500, color: CLASS_COLORS[u.cls] || '#f0d080', fontFamily: 'Cinzel,serif', fontSize: 12 }}>{u.name}</td>
                  <td style={{ padding: '.65rem .8rem', fontSize: 11, color: '#5a4828' }}>
                    {u.cls}{u.race ? ` · ${u.race}` : ''}{u.level ? ` · Lvl ${u.level}` : ''}
                  </td>
                  <td style={{ padding: '.65rem .8rem', fontSize: 12, color: '#7a6030' }}>{u.rank}</td>
                  ...
                  <td style={{ padding: '.65rem .8rem', whiteSpace: 'nowrap' }}>
                    <button className="btn-icon" title="Details bearbeiten" onClick={() => setDetailUser(u)}>✏️</button>
                    ...
                    <button className="btn-icon danger" title="Löschen" onClick={async () => {
                      if (window.confirm(`${u.name} wirklich löschen?`)) await deleteUser(u.id)
                    }}>✕</button>
                  </td>
                </tr>
              ))}
      ...
      <div style={{ marginTop: '.8rem', fontSize: 11, color: '#3a2c18', fontStyle: 'italic' }}>{users.length} Benutzer gesamt</div>
