...
function CharEditor({ char, charIdx, total, t, onSave, onDelete, ranks }) {
  ...
  const subTabStyle = (id) => ({
    ...
    textTransform: 'uppercase', padding: '4px 10px', marginBottom: -1,
    transition: 'all .15s',
  })
    ...
    <div style={{ border: `1px solid ${open ? t.accent + '50' : t.accentFade}`, borderRadius: 3, overflow: 'hidden', transition: 'all .2s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0.7rem 0.9rem', background: open ? `${t.accent}06` : t.bgDark, cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {saved && <span style={{ fontSize: 9, color: '#4a9a5a', fontStyle: 'italic' }}>✓</span>}
          {total > 1 && (
            <button className="btn-icon danger" style={{ fontSize: 10 }}
              onClick={e => { e.stopPropagation(); if (window.confirm(`${char.name} wirklich löschen?`)) onDelete(charIdx) }}>✕</button>
          )}
        ...
        <div style={{ borderTop: `1px solid ${t.accentFade}`, background: t.cardBg }}>
          ...
          <div style={{ display: 'flex', gap: 2, padding: '0 0.9rem', borderBottom: `1px solid ${t.accentFade}` }}>
            {[['info','Charakter'],['professions','Berufe'],['absence','Abwesenheit']].map(([id, label]) => (
              <button key={id} onClick={() => setActiveTab(id)} style={subTabStyle(id)}>{label}</button>
            ))}
          </div>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {[{id:'main',label:'⭐ Main'},{id:'twink',label:'🔄 Twink'}].map(opt => (
                        <button key={opt.id} onClick={() => setCharType(opt.id)} style={{
                          flex: 1, background: charType === opt.id ? `${t.accent}18` : 'transparent',
                          border: charType === opt.id ? `1px solid ${t.accent}` : `1px solid ${t.accentFade}`,
                          borderRadius: 3, padding: '6px', cursor: 'pointer',
                          fontFamily: 'Cinzel,serif', fontSize: 10, letterSpacing: 1,
                          color: charType === opt.id ? t.accentSoft : t.accentDim, transition: 'all .15s',
                        }}>{opt.label}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  : profs.map((prof, pidx) => {
                      ...
                      return (
                        <div key={pidx} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 1fr auto', gap: 6, alignItems: 'center', padding: '0.4rem 0', borderBottom: `1px solid ${t.accentFade}` }}>
                          ...
                          <button onClick={() => setProfs(profs.filter((_,i) => i !== pidx))} style={{ background: 'transparent', border: 'none', color: t.accentDim, cursor: 'pointer', fontSize: 13 }}
                            onMouseEnter={e => e.target.style.color='#c04040'} onMouseLeave={e => e.target.style.color=t.accentDim}>✕</button>
                        </div>
                      )
                    })
                }
                {(canAddPrimary || canAddSecondary) && (
                  <button className="btn-ghost" style={{ fontSize: 9, marginTop: 8 }} onClick={addProf}>+ Beruf hinzufügen</button>
                )}
...
function UserDetailPanel({ user, ranks, onClose, t }) {
  ...
  async function handleAddChar() {
    ...
    const chars = [...characters, newChar]
    await updateDoc(doc(db, 'users', user.id), { characters: chars })
    setNewCharName(''); setNewCharCls('Warrior'); setAddingChar(false)
    setBusy(false); flash()
  }
  ...
  async function saveRank() {
    setBusy(true); setErr('')
    try { await updateDoc(doc(db, 'users', user.id), { rank }); flash() }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200, padding:'1rem' }}
      ...
      <div style={{ background:t.cardBg, border:`1px solid ${t.accentFade}`, borderRadius:4, width:'100%', maxWidth:600, maxHeight:'90vh', display:'flex', flexDirection:'column', position:'relative' }}>
        ...
        <div style={{ padding:'1.2rem 1.4rem 0', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:'1rem' }}>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:'Cinzel,serif', fontSize:17, color:t.accentSoft, fontWeight:600 }}>
        ...
        <div style={{ padding:'1rem 1.4rem', overflowY:'auto', flex:1 }}>
          ...
          {tab === 'characters' && (
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              ...
              {/* Neuen Charakter hinzufügen */}
              {!addingChar ? (
                <button onClick={() => setAddingChar(true)} style={{
                  marginTop:4, background:'none', border:`1px dashed ${t.accentFade}`,
                  color:t.accentDim, fontFamily:'Cinzel,serif', fontSize:10, letterSpacing:2,
                  padding:'10px', cursor:'pointer', borderRadius:3, textTransform:'uppercase', transition:'all .15s',
                }}
                ...
                <div style={{ border:`1px solid ${t.accentFade}`, borderRadius:3, padding:'0.9rem', background:t.bgMid, display:'flex', flexDirection:'column', gap:10 }}>
                  <div style={{ fontFamily:'Cinzel,serif', fontSize:10, color:t.accentDim, letterSpacing:1, marginBottom:2 }}>Neuer Charakter</div>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                    <div><Lbl t={t}>Charaktername</Lbl>
                      <input value={newCharName} onChange={e=>setNewCharName(e.target.value)} placeholder="z.B. Grauhorn" style={{ fontSize:12, width:'100%', boxSizing:'border-box' }} />
                    </div>
                    <div><Lbl t={t}>Klasse</Lbl>
                      <select value={newCharCls} onChange={e=>setNewCharCls(e.target.value)} style={{ fontSize:12, width:'100%' }}>
                        {WOW_CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                  </div>
                  ...
                  <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                    ...
                    <button className="btn-primary" style={{ fontSize:10 }} onClick={handleAddChar} disabled={busy}>
                      {busy ? '...' : 'Hinzufügen'}
                    </button>
                  </div>
                </div>
          ...
          {tab === 'account' && (
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div style={{ background:t.bgMid, border:`1px solid ${t.accentFade}`, borderRadius:3, padding:'0.8rem', fontSize:12, color:t.textSecondary }}>
                ...
                <div style={{ fontSize:10, color:t.textMuted, marginTop:4, fontStyle:'italic' }}>
                  Benutzername kann nur manuell in Firestore geändert werden.
                </div>
              </div>
              <div>
                <Lbl t={t}>Rang</Lbl>
              ...
              <div style={{ display:'flex', justifyContent:'flex-end' }}>
                <button className="btn-primary" style={{ fontSize:11 }} onClick={saveRank} disabled={busy}>
                  {busy ? 'Speichern...' : 'Rang speichern'}
                </button>
              </div>
            </div>
          )}
          ...
          {tab === 'password' && (
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <p style={{ fontSize:12, color:t.textSecondary, fontStyle:'italic', margin:0 }}>
                Als Admin kannst du das Passwort direkt setzen ohne das alte zu kennen.
              </p>
              <div><Lbl t={t}>Neues Passwort</Lbl>
              ...
              <div style={{ display:'flex', justifyContent:'flex-end' }}>
                <button className="btn-primary" style={{ fontSize:11 }} onClick={savePw} disabled={busy}>
        ...
        {/* Footer */}
        <div style={{ padding:'0.8rem 1.4rem', borderTop:`1px solid ${t.accentFade}`, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
...
export default function Users() {
  ...
  const th = {
    fontFamily:'Cinzel,serif', fontSize:9, letterSpacing:2, color:t.textSecondary,
    textTransform:'uppercase', padding:'.6rem .8rem',
    borderBottom:`1px solid ${t.accentFade}`, textAlign:'left', fontWeight:400, whiteSpace:'nowrap',
  }
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr>{['Status','Benutzername / Charaktere','Rang','Charaktere','Seit','Aktionen'].map(h=><th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={6} style={{ textAlign:'center', padding:'2rem', color:t.accentDim, fontStyle:'italic' }}>
                  {search ? 'Keine Treffer.' : 'Noch keine Benutzer. Leg den ersten an!'}
                </td></tr>
              )}
              {filtered.map(u => {
                const chars = u.characters || []
                const mainChar = chars.find(c => c.characterType === 'main') || chars[0]
                const clsColor = CLASS_COLORS[mainChar?.cls] || t.accentSoft
                return (
                  <tr key={u.id} style={{ borderBottom:`1px solid ${t.accentFade}40`, cursor:'pointer', transition:'background .1s' }}
                    ...
                    <td style={{ padding:'.65rem .8rem', color:t.textMuted, fontSize:11, whiteSpace:'nowrap' }}>
                      {u.createdAt?.toDate?u.createdAt.toDate().toLocaleDateString('de-DE'):'—'}
                    </td>
                    <td style={{ padding:'.65rem .8rem', whiteSpace:'nowrap' }}>
                      <button className="btn-icon" title="Details" onClick={()=>setDetailUser(u)}>✏️</button>
                      <button className="btn-icon" title={u.active?'Deaktivieren':'Aktivieren'} onClick={()=>toggleActive(u.id)}>{u.active?'⏸':'▶'}</button>
                      <button className="btn-icon danger" title="Löschen" onClick={async()=>{ if(window.confirm(`${u.username||u.name} wirklich löschen?`)) await deleteUser(u.id) }}>✕</button>
                    </td>
                  </tr>
                )
              })}
      ...
      {modal === 'add' && (
        <Modal title="Neuen Benutzer anlegen" onClose={()=>setModal(null)} onOk={handleOk} okLabel={busy?'Anlegen...':'Anlegen'}>
          <p style={{ fontSize:12, color:'var(--text-secondary)', fontStyle:'italic', marginBottom:'1rem' }}>
            Der Benutzername wird zum Einloggen verwendet. Der Charaktername ist der Ingame-Name.
          </p>
          <div className="field-group"><label className="field-label">Benutzername (für den Login)</label>
            <input value={form.username} onChange={e=>setForm({...form,username:e.target.value})} placeholder="z.B. Reisebüro" autoComplete="off" />
          </div>
          <div className="field-group"><label className="field-label">Erster Charakter-Name (Ingame)</label>
            <input value={form.charName} onChange={e=>setForm({...form,charName:e.target.value})} placeholder="z.B. Thunderstrike" autoComplete="off" />
          </div>
          ...
          <div className="field-group"><label className="field-label">Klasse (erster Charakter)</label>
