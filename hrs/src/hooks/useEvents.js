...
export function useEvents() {
  ...
  const createEvent = useCallback(async ({ title, type, eventDate, eventTime, description, maxSignups }) => {
    const ref = await addDoc(collection(db, 'events'), {
      title:       title.trim(),
      type,
      eventDate,       // ISO-String 'YYYY-MM-DD'
      eventTime,       // '20:00'
      description:     description?.trim() || '',
      maxSignups:      maxSignups || 0, // 0 = unbegrenzt
      signups:         [],              // [{ userId, username, cls, rank, role, signedUpAt }]
      createdBy:       by,
  ...
  // ── Member: Anmelden ────────────────────────────────────────────────────
  const signUp = useCallback(async (eventId, role) => {
    if (!currentUser) return
    const event = events.find(e => e.id === eventId)
    if (!event) return

    // Bereits angemeldet? → erst abmelden
    const existing = event.signups?.find(s => s.userId === currentUser.id)
    if (existing) {
      await updateDoc(doc(db, 'events', eventId), {
        signups: arrayRemove(existing)
      })
    }
  ...
  return { events, loading, createEvent, deleteEvent, updateEvent, signUp, signOff }
}
